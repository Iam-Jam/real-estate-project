<h2 class="text-3xl font-semibold mb-6">Purchase Agreements</h2>
@if(isset($purchaseAgreements))
    @if($purchaseAgreements->isEmpty())
        <p class="text-lg">No purchase agreements submitted.</p>
    @else
        <p class="text-lg mb-6">Number of purchase agreements: {{ $purchaseAgreements->count() }}</p>
            ...
            @foreach($purchaseAgreements as $agreement)
                <div class="bg-white bg-opacity-25 rounded-lg shadow-lg p-6 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-105">
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('purchase-agreements.show', $agreement) }}"
    ...
    @endif
@else
    <p class="text-lg">Purchase agreements data not available.</p>
@endif
